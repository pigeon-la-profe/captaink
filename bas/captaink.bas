220DEFINTA-Z:le=0:pu=0:DIMHI(14,3):km=0:kn=0:ks=0:ld=0:h4=0:ck=27:lk=0:pm=0:p1=0:p2=0:py=0:px=0:pz=0:ph=3
300DIMPL(14,3):DIMC0(1,9):DIMC1(6,2):DIMM0(1,4):DIMM1(4,2):DIMR1(4,3):DIMC(100):ld=0
400'VDP(1)=VDP(1)AND191'IFlo=le THENGOSUB30000:GOTO3000
500OPEN"levels10.dat"AS#1LEN=4:FIELD #1,2 as b$:FORi=1to100:get#1,i+le*(100):C(i-1)=CVI(b$):NEXTi:CLOSE#1:GOSUB20000:GOSUB30000
510ERASEPL,C0,C1,M0,M1,R1,C:VDP(1)=VDP(1)OR64
3000_turboon(le,pu,HI(),lk,km,kn,ks,ld,h4,ck,pm,p1,p2,py,px,pz,ph)
3010DIMpt(5):DIMTL(4):DIMCL(8):DIMRE(6):DIMRY(10):DIMTK(3):DIMTR(6):DIMCF(6,2)
3020VPOKE6144+31,ks:vdp(4)=0:vdp(3)=159:R=RND(-TIME):IFPEEK(&H002B)>128THENde=3000ELSEde=3600
...
4000'mloop
4010S=STICK(0):IFS=0THENS=STICK(1)
5200IFS=3ORS=2ORS=4THENPO=0:ur=ur+1:FA=0
5210IFS=7ORS=5ORS=8THENPO=1:ur=ur+1:FA=1
...
7500IFS1=-1ANDsa=0AND(CL(0)=1ORp6=1)THENGOSUB8000ELSEIFsa=1THENGOSUB8117
...
11000FORI=0TO5:TR(I)=I:NEXTI:y0=0:y2=0:tj=0:FORI=0TO767
...
14325GOSUB14720
14400'IFdeMOD2<>0GOTO14460'CTARJ
...
14430IFmc=96THENmc=0:VPOKE6144+7+cp+22*32,59:VPOKECF(cp,0)+1-1*32,CF(cp,1):CF(cp,1)=10:tc=10:ta=0:cp=0:sound9,0:wi=wi+1
14440IFwi=6THENpu=pu+de:T$=STR$(pu):tx%=25:ty%=22:tq=7:GOSUB9990:T1$=" YOU"+" "+"WIN":GOTO15100
14450IFde<=0THENT1$=" YOU"+" "+"LOSE":GOTO15100
14460RETURN
14470T$=STR$(le+1):IF(le)<10THENT$=" 0"+RIGHT$(T$,1)
...
14780VPOKETK(2),TK(0):tp=0:ta=0:tb=0:VPOKE6144+14+32*22,94:RETURN
...
15120IFwi=6THENle=le+1:IFle>19THENle=0
15130GOTO18900
...
18250py=pz:ps=0:pg=1:RETURN
18270FORI=0TO2:VPOKE6144+px+py*32+I,p2-ps:IFpy>phTHENVPOKE6144+px+(py-1)*32+I,p1+ps
...
18900_turbooff
19000GOTO300
20000_turboon(PL(),C0(),C1(),M0(),M1(),R1(),le,pu,C(),lk,km,kn,ks,ld,h4,ck,pm,p1,p2,py,px,pz,ph)
20010ix=0:GOTO20100
...
20155GOSUB20020:km=v:GOSUB20020:kn=v:GOSUB20020:ks=v:GOSUB20020:ld=v:GOSUB20020:h4=v:GOSUB20020:ck=v:GOSUB20020:lk=v:GOSUB20020:pm=v:GOSUB20020:p1=v:GOSUB20020:p2=v:GOSUB20020:py=v:GOSUB20020:px=v:pz=py:GOSUB20020:ph=v
20160_turbooff
20170RETURN
30000_turboon(PL(),C0(),C1(),M0(),M1(),R1(),le,HI(),lk,km,kn,ks,ld,h4,ck,pm)
30005FORI=0TO5:VPOKE6144+7+I+32*22,0:VPOKE6144+7+I+32*23,0:NEXTI:VPOKE6144+15+32*22,93:VPOKE6144+14+32*22,94
