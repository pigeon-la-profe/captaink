...
110hm=0:POKEVAL("&H"+HEX$(VARPTR(hm))),PEEK(&HFC4A):POKEVAL("&H"+HEX$(VARPTR(hm)+1)),PEEK(&HFC4B):SOUND7,191:SOUND8,0:SOUND9,0:SOUND10,0
900le=0:pu=0:DIMHI(14,3):km=0:kn=0:ks=0:ld=0:h4=0:ck=27:lk=0:pm=0:p1=0:p2=0:py=0:px=0:pz=0:ph=3:cs=11:wi=0:tl=0:GOSUB30800:vdp(4)=0:vdp(3)=159:IFPEEK(&H002B)>128THENde=3000ELSEde=3600
3000_turboon(le,pu,HI(),lk,km,kn,ks,ld,h4,ck,pm,p1,p2,py,px,pz,ph,cs,de,wi,tl)
3010DIMpt(5):DIMCL(8):DIMTK(2):DIMTR(6):DIMCF(6,2)
3040CX=16:CY=-24:pt(1)=12:pt(2)=24:pt(3)=36:pt(4)=30:nu=1:rt=5:tc=10:tp=0:ta=0:et=0:tt=128:tf=1:tb=0:R=RND(-TIME):GOSUB11000:T$=STR$(pu):tx=25:ty=22:tq=7:GOSUB9990
3050GOSUB4000
3100IFTIME>2THENGOSUB14100
3110GOTO3050
4000S=STICK(0):IFS=0THENS=STICK(1)'mloop
5200IFS=3ORS=2ORS=4THENPO=0:ur=ur+1:FA=0
5210IFS=7ORS=6ORS=8THENPO=1:ur=ur+1:FA=1
5220IFur>10-ix*2ANDPO=0ANDsa=0ANDCL(2)=0THENCX=CX+ix:GOSUB14000ELSEIFur>10-ix*2ANDPO=0ANDsa=0THENGOSUB14030
5225IFur>10-ix*2ANDPO=1ANDsa=0ANDCL(3)=0THENCX=CX-ix:GOSUB14000ELSEIFur>10-ix*2ANDPO=1ANDsa=0THENGOSUB14050
5227ifOP<>POTHENix=1:iy=0
5230IFS=0THENPO=2:vr=vr+1
5235IFS=1ORSTRIG(3)=-1THENPO=3:vr=vr+1
5239IFCL(0)=0THENGOSUB7800
5250IFvr>100AND(PO=2ORPO=3)THENpa=pa+3:pa=paMOD6:vr=0:GOSUB9500
...
7540RETURN
7800AR=AR+1:IFCL(4)=1ORCL(7)=1OR(pd>=0ANDpd<14ANDpe)THENsp=1ELSEsp=3'GR
7820IFAR>1ANDsa=0ANDp6=0THENCY=CY+sp:AR=0:it=FA*3:PO=4:GOSUB9600:IFCL(0)=1THENix=1:si=1:su=5:fe=2:GOSUB14970
7830RETURN
8000'SA
...
8118IFCL(4)=1ORCL(7)=1OR(pd>=0ANDpd<14ANDpe)THENim=-1ELSEim=-3
8119IFp6=1THENim=0
8120sr=sr+1:IFsr>2THENfr=fr+1:CY=CY-im:CY=CYMOD192:sr=0ELSEGOTO8160
8130IFco=1ANDCL(2)=0THENCX=CX+2:it=0:GOSUB13900
8140IFco=2ANDCL(3)=0THENCX=CX-2:it=0:GOSUB13900
8150GOSUB9600
8160IFfr<3ANDCL(1)=0GOTO8200
8170IF(fr>szORCL(0)=1ORCL(1)=1ORCL(2)=1ORCL(3)=1)THENix=1:fr=0:sr=0:fr=0:co=0:sa=0:su=5:si=1:fe=2:IFfr<=szTHENGOSUB14970
8200RETURN
...
9700T1=(CX+4)\8:T2=(CY+17)\8:GOSUB9840:C1=C0:T1=(CX+6)\8:T2=(CY+4)\8:GOSUB9840:C2=C0'col
9732T1=(CX+12)\8:T2=(CY+17)\8:GOSUB9840:C8=C0:T1=(CX+10)\8:T2=(CY+4)\8::GOSUB9840:C9=C0
...
9230RETURN
9840IFT2<0THENC0=0:RETURN
9845IFt1>31ORt1<0THENC0=0:RETURN
9850C0=VPEEK(6144+T1+T2*32):RETURN
9990L=LEN(T$)-1:T$=RIGHT$(T$,L):OU$=STRING$(tq-L," ")+T$:FORI=1TOLEN(OU$):CT$=MID$(OU$,I,1):VPOKE6144+tx+ty*32+I-1,ASC(CT$)-32:NEXTI:RETURN
11000FORI=0TO5:TR(I)=I:NEXTI:y0=0:y2=0:tj=0:FORI=0TO767:IFVPEEK(6144+I)=ckTHENGOSUB14640:CF(tj,1)=tj+4:CF(tj,0)=I+6144:y2=y2+1
11220IFy2>5THENI=767
11230NEXTI:FORI=0TO5:TR(I)=I+4:NEXTI:rt=5:RETURN
13900IFCX>240THENCX=240
13920IFCX<0THENCX=0:RETURNELSERETURN
14000OP=PO:it=it+3:it=itmod12:ur=0:iy=iy+1:IFiy>2THENix=ix+1:iy=0
14002IFix>3THENix=3
14010gosub13900:GOSUB9600:GOSUB14900:return
14030IF(C3>kmANDC3<kn)AND(C4<=kmORC4>=kn)ANDCX<240THENCX=CX+4:CY=CY-8:ix=1:GOSUB14000:RETURNELSERETURN
14050IF(C6>kmANDC6<kn)AND(C5<=kmORC5>=kn)THENCX=CX-4:CY=CY-8:ix=1:GOSUB14000:RETURNELSERETURN
...
14150IFtm>4ANDh4=2THENFORI=0TO7:VPOKE8192+lK*8+I,RND(1)*16:NEXTI
14160IFtm>4THENtm=0
14190tn=tn+1:IFtn<2THENGOTO14275
14200FORI=0TO7:w4=VPEEK(6188+I):IF(w4>=73ANDw4<=76)THENVPOKE6188+I,73+y1
14210NEXTI:tn=0:y1=y1+1:IFy1>3THENy1=0
14275'tarj
14289ON(tp+1)\1GOTO14290,14300,14325
...
14425GOSUB14470:IFmc=0andCL(6)=1andPO=3thenGOSUB14950
14430IFmc=96THENmc=0:VPOKE6144+7+cp+22*32,59:VPOKECF(cp,0)+1-1*32,CF(cp,1):CF(cp,1)=11:tc=10:ta=0:cp=0:sound9,0:wi=wi+1
14440IFwi=6THENpu=pu+de:CY=209:GOSUB9600:GOTO18900
14450IFde<=0THENCY=209:GOSUB9600:GOTO18900
14460RETURN
14470T$=STR$(le+1):IF(le+1)<10THENT$=" 00"+RIGHT$(T$,1)ELSEIF(le+1)>10AND(le+1)<100THENT$=" 0"+RIGHT$(T$,2)
14480tx=14:ty=23:tq=3:GOSUB9990:IFmc>0GOTO14510
14485IFtp=2THENtz=tb:tw=tt*tfELSEtz=ta:tw=tt
...
14510IFws>16THENVPOKE6144+19+32*22,104ELSEVPOKE6144+19+32*22,96+ws\2
14520IFws<=16THENVPOKE6144+18+32*22,96ELSEVPOKE6144+18+32*22,96+(ws-16)\2
14521IFmc=0THENRETURN
14522IFmc>48THENVPOKE6144+19+32*23,104ELSEVPOKE6144+19+32*23,96+mc\6
14524IFmc<=48THENVPOKE6144+18+32*23,96ELSEVPOKE6144+18+32*23,96+(mc-48)\6
14530RETURN
14540IF(te=90ORte=91)ANDet=2THENte=e1:TK(0)=e1ELSEIF(te=88ORte=89)ANDdt=2THENte=d1:TK(0)=d1
14600RETURN
14640IFrt>=0THENtk=(INT(RND(1)*(rt+1))):tj=TR(tk):rt=rt-1:FORE=tkTO4:TR(E)=TR(E+1):NEXTE
14650RETURN
14720'BrTARJ
...
17000ON(h4+1)\1GOTO18000,17020,17500'hide plat
17020h1=h1+1:IFh1>30THENh1=0:h4=2ELSERETURN
17030h2=(INT(RND(TIME)*(14))):n3=HI(h2,2):n1=HI(h2,0):IFn3<>1THENn3=n3+2ELSEn1=n1+1
17040FORI=0TOn3-1:VPOKE6144+n1+I+32*HI(h2,1),lk:NEXTI:h4=2:IFNOT(mc>0ANDPO=3)THENki=1:ku=10:si=1:su=5:GOSUB14965:RETURN
...
18525IFde<1500ANDde>800ANDwi<3ANDet<>1THENGOTO18530ELSERETURN
...
18650RETURN
18900_turbooff
18910_turboon(wi,le,tl)
18920SOUND7,191:FORI=6TO26:VPOKE6144+I+7*32,119:VPOKE6144+I+14*32,124:NEXTI:FORI=6TO26:FORE=8TO13:VPOKE6144+I+E*32,0:NEXTE:NEXTI:VPOKE6144+5+7*32,118:VPOKE6144+27+7*32,120:VPOKE6144+5+14*32,123:VPOKE6144+27+14*32,125
18925FORI=8TO13:VPOKE6144+5+I*32,121:VPOKE6144+27+I*32,122:NEXTI
18930IFwi=6THENle=le+1:RESTORE18982ELSERESTORE18983
18940IFle>tlTHENle=0
18950FORI=12TO19:READE:VPOKE6144+I+32*10,E:NEXTI:RESTORE18981:FORI=12TO21:READE:VPOKE6144+I+32*12,E:NEXTI
18960kb$=INKEY$:IFkb$<>"c"ANDkb$<>"C"GOTO18960
18970RESTORE18980:FORI=12TO23:READE:VPOKE6144+I+32*10,E:NEXTI:FORI=12TO25:VPOKE6144+I+32*12,0:NEXTI:GOTO18990
18980DATA0,44,47,33,36,41,46,39,0,0,0,0
18981DATA35,0,35,47,46,52,41,46,53,37
18982DATA57,47,53,0,55,41,46,0
18983DATA57,47,53,0,44,47,51,37
18990_turbooff
19000POKEhm,le:POKEhm+2,PEEK(VARPTR(pu)):POKEhm+3,PEEK(VARPTR(pu)+1):run"loadlvc.bas"
30800_turboon(le,pu,HI(),lk,km,kn,ks,ld,h4,ck,pm,p1,p2,py,px,pz,ph,hm,cs,tl)
40000le=PEEK(hm):tl=PEEK(hm+1):pu=PEEK(hm+2)+256*PEEK(hm+3):lk=PEEK(hm+4):km=PEEK(hm+6):kn=PEEK(hm+8):ks=PEEK(hm+10):ld=PEEK(hm+12):h4=PEEK(hm+14)
