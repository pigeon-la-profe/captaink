...
30BLOAD"splashdi.sc2",S:VDP(1)=VDP(1)OR64:tl=257:le=0
40_turboon(le)
42DIMch(17)
45:R=RND(-TIME)
50TIME=0
...
90IFkb$="L"THENle=le+10:RETURNELSEGOTO95
95IFkb$="H"THENle=le+100:RETURNELSEGOTO600
...
750'POKEtp,le:POKEtp+1,tl:POKEtp+2,0:POKEtp+3,0:POKEtp+77,0:LOAD"loadlvc.bas",r
751'POKE tp,PEEK(VARPTR(le)):POKE tp+1,PEEK(VARPTR(le)+1):POKE tp+2,PEEK(VARPTR(tl)):POKE tp+3,PEEK(VARPTR(tl)+1):POKEtp+2,0:POKEtp+3,0:POKEtp+77,0:LOAD"loadlvc.bas",r
752POKE tp,PEEK(VARPTR(le)):POKE tp+1,PEEK(VARPTR(le)+1):POKE tp+2,PEEK(VARPTR(tl)):POKE tp+3,PEEK(VARPTR(tl)+1):POKEtp+77,0:LOAD"loadlvc.bas",r
